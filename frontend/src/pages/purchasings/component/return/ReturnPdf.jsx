...
import { fetchPurchasePDFDetails, fetchReturnPurchasePDFDetails } from "../../../../api/purchasingAPI";
import { getSettings } from "../../../../api/settingsApi";

const ReturnPdf = ({ invoiceId, onClose }) => {
  const pdfRef = useRef();
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const result = await fetchReturnPurchasePDFDetails(invoiceId);
  ...
  const renderPdf = () => {
    ...
    height += 5;
    const dateObject = new Date(purchase.purchaseReturnDate);
        ...
        <Table id="my-table" style={{ display: "none" }}>
          ...
          <Tbody>
            {products.map((product, index) => (
              <Tr key={index}>
                <Td>{product.purchaseReturnItemId}</Td>
