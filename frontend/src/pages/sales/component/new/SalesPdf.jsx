...
const SalesPdf = ({ selectedItemID, onClose }) => {
  ...
  const renderPdf = () => {
    ...
    doc.setFont("helvetica", "bold");
    doc.setFontSize(12);
    doc.text("CASH INVOICE", 10, height);
    height += 15;
    ...
    doc.setFontSize(10);
    doc.text(
      `${formatDate(saleData.sale.saleDate)}`,
      10 + doc.getTextWidth("Invoice #: " + 5),
      height
    );
    height += 10;
    ...
    if (saleData.sale && (saleData.sale.customerPhoneNumber || saleData.sale.customerAddress)) {
      doc.setFont("helvetica", "bold");
      doc.setFontSize(10);
      ...
      if (saleData.sale && saleData.sale.customerPhoneNumber) {
        doc.text(
      ...
      // Check if customer address is available
      if (saleData.sale && saleData.sale.customerAddress) {
        doc.text(
      ...
      // Display empty lines if no data is available
