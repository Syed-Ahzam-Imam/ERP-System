...
const GeneralLedger = ({ selectedItem, branches, onClose }) => {
  ...
  useEffect(() => {
    if (selectedItem && selectedItem.supplierId) {
      const fetchCustomerData = async () => {
        try {
          setLoading(true);
          const response = await getSupplierLedger(selectedItem.supplierId);
  ...
  const ledgerName = `Detailed Ledger Report ${supplierData?.supplierId}|${supplierData?.firstName}`;
  ...
  const handlePdfPrint = () => {
    printPdf(supplierData, filteredEntries, startDate, endDate, ledgerName, settings);
  };
          ...
          <VStack>
            ...
            <Flex direction="column" gap={6}>
              <SimpleGrid columns={2} gap={10} alignItems="center">
                ...
                <Text>{supplierData?.supplierId}</Text>
              </SimpleGrid>
              <SimpleGrid columns={2} gap={10} alignItems="center">
                <Text>Supplier Name:</Text>
                <Text>{supplierData.firstName}</Text>
              </SimpleGrid>
            </Flex>
          </VStack>
        </Box>
