...
import { getProductLedger } from "../../../api/productAPI";
import { generalLedgerDetailedProductsPdf, printProductsPdf } from "../../../utlis/GeneralLedgerDetailedProducts";
...
const GeneralLedger = ({ selectedItem, branches, onClose }) => {
  ...
  useEffect(() => {
    if (selectedItem && selectedItem.productId) {
      const fetchProductData = async () => {
        try {
          setLoading(true);
          const response = await getProductLedger(selectedItem.productId);
  ...
  const ledgerName = `Detailed Ledger Report ${productData?.productName}`;
  ...
  const handlePdfDownload = () => {
    generalLedgerDetailedProductsPdf(
      productData,
      filteredEntries,
      startDate,
      endDate,
  ...
  const handlePdfPrint = () => {
    printProductsPdf(productData, filteredEntries, startDate, endDate, ledgerName, settings);
  };
          ...
          <VStack>
            ...
            <Flex direction="column" gap={6}>
              <SimpleGrid columns={2} gap={10} alignItems="center">
                ...
                <Text>{productData?.productId}</Text>
              </SimpleGrid>
              <SimpleGrid columns={2} gap={10} alignItems="center">
