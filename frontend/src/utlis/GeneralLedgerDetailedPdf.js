...
const generatePdf = (customerDetails, ledgerDetails, startDate, endDate, ledgerName, settings) => {
    ...
    } = calculateFinalBalance(ledgerDetails);

    // console.log("-------------------------BALNCAE", paymentDetailsWithBalance);
    ...
    doc.setFontSize(15);
    doc.text(`${customerDetails?.customerId ? 'Customer Name' : 'Supplier Name'}: ${customerDetails.firstName}`, 10, height);
    ...
    doc.text(`Total Debit: ${totalReceipt}`, width - doc.getTextWidth(`Total Receipt: ${totalReceipt}`), height);
    doc.text(`Total Credit: ${totalPayment}`, width - doc.getTextWidth(`Total Payment: ${totalPayment}`), height + 5);
...
export const generalLedgerDetailedPdf = (customerDetails, ledgerDetails, startDate, endDate, ledgerName, settings) => {
    const doc = generatePdf(customerDetails, ledgerDetails, startDate, endDate, ledgerName, settings);
    doc.save(`${ledgerName}`)
};
export const printPdf = (customerDetails, ledgerDetails, startDate, endDate, ledgerName, settings) => {
    const doc = generatePdf(customerDetails, ledgerDetails, startDate, endDate, ledgerName, settings);
